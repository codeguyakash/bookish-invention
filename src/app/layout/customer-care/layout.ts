...
export class CustomerCareLayout implements OnInit {
    emp_token: any;
    toggleAside: any;
    togglePanel: any;
    ...
    ngOnInit() {
        ...
        this.allRoles = this.userData.role_ids.split(',');
    ...
    async logout() {
        if(this.sharedService.cceCurrentLeadId) {
            await this.leadPanelService.makeLeadInQueue(+this.sharedService.cceCurrentLeadId).subscribe(data => {
                if(data.status == 1) {
                    console.log("lead released");
                    this.sharedService.cceCurrentLeadId = "";
                } else {
                    console.log("Error in lead release");
