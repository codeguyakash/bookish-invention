...
@Component({
    selector: 'app-site-surveyor-layout',
    templateUrl: './layout.html',
    encapsulation: ViewEncapsulation.None,
    styleUrls: ['../../../assets/css/cc-styles.css', '../../../assets/css/employee-login.css']
})
export class SiteSurveyorLayout implements OnInit {
    emp_token: any;
    toggleAside: any;
    togglePanel: any;
    ...
    ngOnInit() {
        ...
        const financeLegacy = localStorage.getItem('financeLegacy');
        if(financeLegacy && financeLegacy == 'true') {
          this.finaceLegacy = true;
        } else {
          this.finaceLegacy = false;
        }
    }
    ...
    rightMove() {
        if (localStorage.getItem('emp_auth_key')) {
            if (localStorage.getItem('role_id') == '22') {
            ...
            } else if (localStorage.getItem('role_id') == '9' && localStorage.getItem('financeLegacy') != 'true') {
                this.router.navigateByUrl('/employee-panel/lead-list-finance');
            } else if (localStorage.getItem('role_id') == '9' && localStorage.getItem('financeLegacy') == 'true') {
              this.router.navigateByUrl('/employee-panel/lead-list-finance-legacy');
              ...
              this.router.navigateByUrl('/employee-panel/followup');
            ...
            this.subscription = this.leadListService.checkRoleExists(roleId).subscribe(data => {
                if (data.status) {
                    ...
                    this.router.navigateByUrl('/employee-panel/project-manager');
                        ...
                        this.router.navigateByUrl('/employee-panel/supply-order');
                    }
