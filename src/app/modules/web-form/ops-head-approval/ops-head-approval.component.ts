...
@Component({
  selector: "app-ops-head-approval",
  templateUrl: "./ops-head-approval.component.html",
  styleUrls: ["./ops-head-approval.component.css"],
})
export class OpsHeadApproval implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getSspPaymenToApproveList();
  }

  getSspPaymenToApproveList(page = 1) {
    this.loader = true;
    ...
    this.Listing = "OpsHeadApproval";
    this.sharedServices.searchForm.next("OpsHeadApproval");
    this.leadListService
      .getSspPaymentApproval(page, this.itemPerPage, "PR", "", "", "OH", true)
      .subscribe(
        (data) => {
          if (data.status == 1) {
  ...
  rejectPayment(event, payment) {
    ...
    if (event.target.checked) {
      ...
      const index = this.rejectedPaymentsPtaskId.findIndex(
        (x) => x.id == ptaskId
      );
      if (index >= 0) {
  ...
  submitApproval() {
    this.loader = true;
    if (
      this.approvedPaymentsPtaskId.length ||
      this.rejectedPaymentsPtaskId.length
    ) {
      const data = {
        ptask_Id: this.approvedPaymentsPtaskId,
        ptask_Id_reject: this.rejectedPaymentsPtaskId,
      };
      this.leadListService.aproveSspPayrequestOpshead(data).subscribe(
        (res) => {
        if (res.status) {
          ...
          this.popupMessageService.showError(
            "Can't approve SSP payment request",
            "Error"
          );
        }
        this.loader = false;
      ...
      this.popupMessageService.showInfo(
        "Please approve/reject atleast one request to continue or click Cancel",
        "Info"
      );
    }
  }
  ...
  smartSearchResultsApproved($event) {
    ...
    if ($event.status == 1) {
      this.paymentHistory = [];
      if (this.Listing == "OpsheadApprovedPayments") {
  ...
  rejectAll(event) {
    if (event.target.checked) {
      this.paymentRequestList.forEach((item) => {
        item.isReject = 1;
        item.isApproved = 0;
        var element = <HTMLInputElement>document.getElementById("approveAll");
  ...
  getPaymentHistory(ptaskId) {
    this.leadListService.getPaymentHistory(ptaskId).subscribe((res) => {
      console.log(res);
  ...
  getApprovedPayments(page=1) {
    ...
    this.Listing = "OpsheadApprovedPayments";
    this.sharedServices.searchForm.next("OpsheadApprovedPayments");
    const data = {
      type: 1
    }
  ...
  pageSubmit() {
    if (this.paginationSubmitForm.valid) {
        ...
        if(this.Listing == "OpsHeadApproval") {
          this.getSspPaymenToApproveList(page);
        } else {
